...
\definecolor{QAMPOcore}{HTML}{ACE8AE}
\definecolor{PTInstrument}{HTML}{F6E8A6}
\definecolor{PTInstIn}{HTML}{F7E58B}
\definecolor{PTInstOut}{HTML}{FCB06D}
\definecolor{PTInstMap}{HTML}{AD9986}
...
% Fallback only; \PTTensor computes a size-relative radius from Fig. 8
% (1.2mm on an 11mm square, 0.8mm on an 11mm-tall triangle).
...
% Fig. 8 reference ratios: square 1.2/11, triangle 0.8/11, of min(width,height).
\newcommand{\PTSetSquareRound}[2]{%
  \setlength{\PTRndCmpA}{#1}%
  \setlength{\PTRndCmpB}{#2}%
  \ifdim\PTRndCmpA>\PTRndCmpB
    \pgfmathsetlength{\PTRelRound}{(1.2/11)*\PTRndCmpB}%
  \else
    \pgfmathsetlength{\PTRelRound}{(1.2/11)*\PTRndCmpA}%
  \fi
}
\newcommand{\PTSetTriangleRound}[2]{%
  \setlength{\PTRndCmpA}{#1}%
  \setlength{\PTRndCmpB}{#2}%
  \ifdim\PTRndCmpA>\PTRndCmpB
    \pgfmathsetlength{\PTRelRound}{(0.8/11)*\PTRndCmpB}%
  \else
    \pgfmathsetlength{\PTRelRound}{(0.8/11)*\PTRndCmpA}%
  \fi
}
\newlength{\PTTraceNodeRound}
\PTSetSquareRound{7.2mm}{7.2mm}
\setlength{\PTTraceNodeRound}{\PTRelRound}
...
\tikzset{
  ...
  % Prime tick at mid-stub: a thin, slightly tall slash through both rails.
  PTLiouvilleLeg primed/.style={
    PTLiouvilleLeg,
    postaction={
      decorate,
      decoration={
        ...
        mark=at position 0.5 with {
          \fill[white] (-0.28mm,-1.12mm) rectangle (0.62mm,1.20mm);
          \draw[
            PTLabel,
            line width=0.45pt,
            line cap=round,
          ] (-0.18mm,-0.98mm) -- (0.38mm,1.06mm);
        }
      }
    },
  },
  ...
  PTTraceNode/.style={
    draw=black,
    fill=PTTrace!18,
    rounded corners=\PTTraceNodeRound,
    minimum width=7.2mm,
...
\newcommand{\PTKet}[3]{%
  ...
  \begin{scope}[on PTforeground layer]
    \PTSetTriangleRound{\PTKetWidth}{\PTKetHeight}
    \filldraw[
      draw=black,
      fill=PTHilbertKet!18,
      line width=\PTCoreStroke,
      line join=round,
      rounded corners=\PTRelRound,
...
% Output Liouville ket when time runs right-to-left: left-pointing triangle,
% wire attached at .east (same geometry as a Liouville bra).
\newcommand{\PTLiouvilleKetLeft}[3]{\PTLiouvilleBra{#1}{#2}{#3}}
...
\newcommand{\PTTensor}[4][]{%
  ...
  \def\PTTensorRnd{PTAUTO}%
  \def\PTTensorFont{\small}%
  \setkeys{pttensor}{#1}%
  \def\PTAuto{PTAUTO}%
  \ifx\PTTensorRnd\PTAuto
    \PTSetSquareRound{\PTTensorMinW}{\PTTensorMinH}%
  \else
    \setlength{\PTRelRound}{\PTTensorRnd}%
  \fi
  \begin{scope}[on PTforeground layer]
    \node[
      name=#2,
...
\newcommand{\PTInstrument}[4][]{%
  \PTTensor[fill=PTInstrument,minimum width=\PTCoreWidth,minimum height=\PTCoreWidth,#1]{#2}{#3}{#4}%
}
...
\newcommand{\PTCombiner}[4][]{%
  ...
  \else
    \def\PTCombRot{0}%
  \fi\fi\fi
  \PTSetTriangleRound{\PTCombinerSize}{\PTCombinerSize}%
  \begin{scope}[on PTforeground layer]
    \node[
      name=#2,
      isosceles triangle,
...
% Same loop, but above the core (used for Tr(A) and Tr(O rho) in Fig. 2).
\newcommand{\PTTraceLoopAbove}[2][PTWire]{%
  \draw[#1]
    (#2.west)
    arc[start angle=270, delta angle=-180, radius=4.0mm]
    -- ([yshift=8.0mm]#2.east)
    arc[start angle=90, delta angle=-180, radius=4.0mm];
  \coordinate (#2-looptop) at ([yshift=8.0mm]#2.center);
}

% Gradual S-dip: horizontal span \PTVecDipH, drop 2 R - fuse gap.
\makeatletter
% Bend the east (bra) wire over the top, then dip it down next to the
% ket so the pair reads as a fused Liouville leg.  |A>>
% #1-ket = lower left (j), #1-bra = upper left (k).
\newcommand{\PTVectoriseToKet}[1]{%
  \draw[PTHilbertBraLeg]
    (#1.east)
    arc[start angle=270, delta angle=90, radius=\PTVecBendR]
    arc[start angle=0, delta angle=90, radius=\PTVecBendR]
    -- ($ (#1.west) + (-\PTVecOverhang, 2*\PTVecBendR) $)
    .. controls
      ($ (#1.west) + (-\PTVecOverhang-0.42*\PTVecDipH, 2*\PTVecBendR) $)
      and
      ($ (#1.west) + (-\PTVecOverhang-0.58*\PTVecDipH, \PTFuseGap) $) ..
    ($ (#1.west) + (-\PTVecOverhang-\PTVecDipH, \PTFuseGap) $)
    -- ++(-\PTVecRun,0) coordinate (#1-bra);
  \draw[PTHilbertKetLeg]
    (#1.west) -- (#1-bra |- #1.west) coordinate (#1-ket);
}

% Bend a west-port wire over the top (left-bulging semicircle), then dip
% it down next to the east port: <<A| or <<I|.
% #1 = name, #2 = west coordinate, #3 = east coordinate.
% #1-bra = lower right (k), #1-ket = upper right (j).
\newcommand{\PTVectorisePortsToBra}[3]{%
  \draw[PTHilbertKetLeg]
    (#2)
    arc[start angle=270, delta angle=-90, radius=\PTVecBendR]
    arc[start angle=180, delta angle=-90, radius=\PTVecBendR]
    -- ($ (#3) + (\PTVecOverhang, 2*\PTVecBendR) $)
    .. controls
      ($ (#3) + (\PTVecOverhang+0.42*\PTVecDipH, 2*\PTVecBendR) $)
      and
      ($ (#3) + (\PTVecOverhang+0.58*\PTVecDipH, \PTFuseGap) $) ..
    ($ (#3) + (\PTVecOverhang+\PTVecDipH, \PTFuseGap) $)
    -- ++(\PTVecRun,0) coordinate (#1-ket);
  \draw[PTHilbertBraLeg]
    (#3) -- (#1-ket |- #3) coordinate (#1-bra);
}
\newcommand{\PTVectoriseToBra}[1]{%
  \PTVectorisePortsToBra{#1}{#1.west}{#1.east}%
}
% Identity as a wire: left-bulging cup, then dip immediately (no extra
% straight run after the semicircle).  #1 = name, #2 = west coordinate.
\newcommand{\PTVectoriseIdentityToBra}[2]{%
  \draw[PTHilbertKetLeg]
    (#2)
    arc[start angle=270, delta angle=-90, radius=\PTVecBendR]
    arc[start angle=180, delta angle=-90, radius=\PTVecBendR]
    .. controls
      ($ (#2) + (0.42*\PTVecDipH, 2*\PTVecBendR) $)
      and
      ($ (#2) + (0.58*\PTVecDipH, \PTFuseGap) $) ..
    ($ (#2) + (\PTVecDipH, \PTFuseGap) $)
    -- ++(\PTVecRun,0) coordinate (#1-ket);
  \draw[PTHilbertBraLeg]
    (#2) -- (#1-ket |- #2) coordinate (#1-bra);
}
\makeatother

% Compact trace-out: a short Liouville stub capped on the open (west) end
% by a straight slash at site-leg thickness.  The midpoint sits on the
% wire; the tick is drawn in front so it hides the double-wire butt.
% Anchors: #1, #1-west (slashed tip), #1-east (attachment).
\newcommand{\PTTraceOutTick}[1]{%
  \begin{scope}[on PTforeground layer, shift={(#1)}, rotate=-34]
    \draw[
      black,
      line cap=round,
      line width=\PTHilbertLW,
...
\newcommand{\PTTraceOut}[2]{%
  \coordinate (#1) at #2;
  \coordinate (#1-west) at ($ #2 + (-2.8mm,0) $);
  \coordinate (#1-east) at ($ #2 + (2.4mm,0) $);
  \draw[PTLiouvilleLeg]
    (#1-west) -- (#1-east);
  \PTTraceOutTick{#1-west}
}
...
\define@key{ptinst}{rounded}{\def\PTInstRnd{#1}}
\makeatother
...
\newcommand{\PTInstrumentNode}[4][]{%
  ...
  \else
    ...
    \else
      ...
      \else
        \def\PTAuto{PTAUTO}%
        \ifx\PTInstRnd\PTAuto
          \PTSetSquareRound{\PTInstMinW}{\PTInstMinH}%
        \else
          \setlength{\PTRelRound}{\PTInstRnd}%
        \fi
        \begin{scope}[on PTforeground layer]
          \node[
            name=#2,
